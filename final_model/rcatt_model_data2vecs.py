...
def corpus_embedding(filePath,length):
    file = open(filePath)
    rnn_data = np.empty((length,rnn_step,embedding_length),dtype="float64")
    ...
    for line in file:
        ...
        rnn_data[index] = rnnDataEmbedding(wordsList,name1,name2)
        cnn_data[index] = cnnDataEmbedding(wordsList,name1,name2)
...
def store_embedding():
    ...
    np.save("../rcatt_train_rnn_data.npy",train_rnn_data)
    np.save("../rcatt_train_cnn_data.npy",train_cnn_data)
    np.save("../rcatt_train_label.npy",train_label)

    test_rnn_data,test_cnn_data,test_label = corpus_embedding(testFilePath,1037)
    np.save("../rcatt_test_rnn_data.npy",test_rnn_data)
    np.save("../rcatt_test_cnn_data.npy",test_cnn_data)
    np.save("../rcatt_test_label.npy",test_label)
