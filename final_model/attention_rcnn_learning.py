...
rnn_length = embedding_length     #rnn部分的单词数   4*5, 最多4个人名,每个人名包括前后共5个词
window = 3  # cnn滑动窗口大小
...
def read_train_data():
    ...
    train_rnn_data = np.load("../attention_train_rnn_data.npy")
    train_cnn_data = np.load("../attention_train_cnn_data.npy")
    train_label = np.load("../attention_train_label.npy")
    return train_rnn_data,train_cnn_data,train_label

def read_test_data():

    test_rnn_data = np.load("../attention_test_rnn_data.npy")
    test_cnn_data = np.load("../attention_test_cnn_data.npy")
    test_label = np.load("../attention_test_label.npy")
...
def attenRCnn(train_lexical_data,train_sentence_data,train_label,
    ...
    rnn_branch.add(Bidirectional(SimpleRNN(500,return_sequences=True),merge_mode='concat',
                      input_shape=(timestep,rnn_length)))
    ...
    cnn_branch.add(Dense(100,activation='tanh',W_regularizer=l2(0.01)))
