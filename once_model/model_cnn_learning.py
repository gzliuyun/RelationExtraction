...
def read_train_data():
    ...
    train_lexical_data = np.load("../cnn_train_lexical_embedding_data.npy")
    train_sentence_data = np.load("../cnn_train_sentence_embedding_data.npy")
    train_label = np.load("../cnn_train_embedding_label.npy")
    return train_lexical_data,train_sentence_data,train_label

def read_test_data():

    test_lexical_data = np.load("../cnn_test_lexical_embedding_data.npy")
    test_sentence_data = np.load("../cnn_test_sentence_embedding_data.npy")
    test_label = np.load("../cnn_test_embedding_label.npy")
...
def cnn(train_lexical_data,train_sentence_data,train_label,
    ...
    # lexical_input = Input(shape=(lexical_length,embedding_length),name='lexical_input')
    # lexical_convolution = Convolution1D(50,3,border_mode='same',input_shape
    #                 =(lexical_length,embedding_length),W_regularizer=l2(0.01))(lexical_input)
    # lexical_maxpoling = GlobalMaxPooling1D()(lexical_convolution)
    # lexical_dropout = Dropout(0.2)(lexical_maxpoling)
    # lexical_output = Dense(50,activation='tanh',W_regularizer=l2(0.01))(lexical_dropout)
    #
    # sentence_input = Input(shape=(timestep,sentence_embedding_length),name='sentence_input')
    # sentence_convolution = Convolution1D(200,3,border_mode='same',input_shape
    #             =(timestep,sentence_embedding_length),W_regularizer=l2(0.01))(sentence_input)
    # sentence_maxpoling = GlobalMaxPooling1D()(sentence_convolution)
    # sentence_dropout = Dropout(0.6)(sentence_maxpoling)
    # sentence_output = Dense(100,activation='tanh',W_regularizer=l2(0.01))(sentence_dropout)
    #
    # feature_extraction = merge([lexical_output,sentence_output],mode='concat')
    # output = Dense(label_count,activation='softmax',name='output')(feature_extraction)
    # final_model = Model(input=[lexical_input,sentence_input], output=output)

    lexical_branch = Sequential()
    lexical_branch.add(Convolution1D(50,3,border_mode='same',input_shape
                    =(lexical_length,embedding_length),W_regularizer=l2(0.01)))
    lexical_branch.add(GlobalMaxPooling1D())
    lexical_branch.add(Dense(50,activation='tanh',W_regularizer=l2(0.01)))

    sentence_branch = Sequential()
    sentence_branch.add(Convolution1D(200,3,border_mode='same',input_shape
                =(timestep,sentence_embedding_length),W_regularizer=l2(0.01)))
    sentence_branch.add(GlobalMaxPooling1D())
    sentence_branch.add(Dropout(0.6))
    sentence_branch.add(Dense(100,activation='tanh',W_regularizer=l2(0.01)))
