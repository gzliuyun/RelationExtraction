...
from keras.layers import Dense, Input,Activation,SimpleRNN,Flatten,merge,Merge
...
def c_r_nn(train_lexical_data,train_sentence_data,train_label,
    ...
    lexical_branch.add(Dense(100,activation='tanh',W_regularizer=l2(0.01)))
    lexical_branch.add(Dropout(0.3))


    sentence_branch = Sequential()
    sentence_branch.add(Bidirectional(SimpleRNN(500,return_sequences=True),merge_mode='concat',
                              input_shape=(timestep,sentence_embedding_length)))
    sentence_branch.add(Activation('tanh'))
    sentence_branch.add(GlobalMaxPooling1D())
    sentence_branch.add(Dense(200,activation='tanh'))
...
if __name__ == "__main__":
     ...
     c_r_nn(train_lexical_data,train_sentence_data,train_label,
         test_lexical_data,test_sentence_data,test_label)
